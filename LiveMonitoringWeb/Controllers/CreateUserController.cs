...
namespace LiveMonitoringWeb.Controllers
{
    ...
    public class CreateUserController : Controller
    {
        ...
        public JsonResult JsonUserData()
        {
            List<UserProfile> ListUserProfile = new List<UserProfile>();
            try
            {
            ...
            ReturnUserData returnData = new ReturnUserData();

            List <webpages_Roles> Userrole=new List<webpages_Roles>();

            var role = System.Web.Security.Roles.GetRolesForUser(WebSecurity.CurrentUserName);
            if (role[0] == "Admin")
            {
                Userrole = db.tbl_webpages_Roles.ToList();
            }
            else
            {
                Userrole = db.tbl_webpages_Roles.ToList();
            ...
            returnData.Userlist = ListUserProfile;
            returnData.UserRoles = Userrole;
